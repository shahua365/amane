...
`stop()` 先置 `_running=False` 并**取消主循环 task**, 再处置活跃任务: `worker.shutdown_timeout` (默认 `0`, 上限 120) 是等待活跃任务自然完成的秒数, `0` 表示立即超时并 cancel. 主循环若不被终止, 停在 DB 往返中的 claim 会在 `stop()` 返回后认领**之后**入队的任务, 因此 API 测试停 worker 必须在此语义下才不竞态.
